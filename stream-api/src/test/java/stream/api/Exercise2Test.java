...
public class Exercise2Test extends ClassicOnlineStore {
	...
	public void descSortByAge() {
		 ...
		 */
		Comparator<Integer> descOrder = (i1, i2) -> i2 - i1;
		Stream<Integer> sortedAgeStream = customerList.stream()
			.map(Customer::getAge)
			.sorted(descOrder);
	...
	public void top3RichCustomer() {
		 ...
		 */
		Comparator<Customer> customerComparator = (customer1, customer2) -> customer2.getBudget() - customer1.getBudget();
	...
	public void itemsCustomersWantToBuy() {
		 ...
		 */
		Function<Customer, Stream<Item>> getItemStream = customer -> customer.getWantToBuy()
			.stream();
		...
		assertThat(itemList,
				contains("small table", "plate", "fork", "ice cream", "screwdriver", "cable", "earphone", "onion", "ice cream", "crisps", "chopsticks", "cable", "speaker",
						"headphone", "saw", "bond", "plane", "bag", "cold medicine", "chair", "desk", "pants", "coat", "cup", "plate", "fork", "spoon", "ointment", "poultice",
						"spinach", "ginseng", "onion"));
	}
}
